...
- 🔴 **F4／F5／F13／F15 四個框的檔豁免本條**（框本身就是形式）。本日豁免檔為：
  `drama-binge-quotes`／`korean-drama-quotes`／`programmer-jokes`（F4）、
  `cat-quotes`／`pet-memorial-quotes`（F5）、
  `healing-quotes`／`sweet-quotes`（F13）、
  `dark-humor`／`food-jokes`（F15）。**這 9 檔照框寫就好，不必湊形式。**
...
- 🔴 **時效指派（只限 3 檔）：教師節「敬師禮金」制度側（9/28 教師節國定假日前一週）。**
  外部素材（台灣側，已查證）：2026 年敬師禮金**沒有全台統一金額**——台南今年由 600 加碼到 **1,000 元**，並擴大到編制外長期代理教師與專職族語教師；
  台北是 **2,000 元等值商品禮券**；台中、高雄、花蓮 2,000；桃園 1,000；雲林、南投 600。
  站內 `老師` 217 則**全是師生關係**，**0 則**把老師寫成「領薪水、領禮券的人」；`敬師`／`家長會` 皆 0。
  | 檔 | 必落詞（≥2 則） | 角度 |
  |---|---|---|
  | `workplace-quotes`（F6 反問收尾） | `禮券` | 老師當受薪者：禮券的使用期限、不能換現金、隔壁縣市多一千 |
  | `witty-comebacks`（F6 反問收尾） | `敬師` | 「你們有教師節獎金嗎？」式的回嗆與反問 |
  | `mom-quotes`（F2 誤會與本意） | `家長會` | 家長側：家長會要不要集資送老師、送什麼才不尷尬 |
  🔴 地雷：⑴ **金額照實**（台南 1,000／台北 2,000 等值禮券／雲林南投 600），**不要寫成全國統一**；
  ⑵ 🔴 **不得引用大陸「教師節送禮新規」**（禁現金、禁轉帳、家委不許集資）——那是大陸制度，台灣不成立；
  ⑶ `謝師宴`／`卡片`／`聯絡簿` 不得當主角；不寫師生感情、不寫「老師辛苦了」式感謝文；
  ⑷ 三檔**各自的必落詞不得出現在別檔**。
  這 3 檔可以用 `教師節`／`老師`；**其餘 39 檔 `教師節`／`敬師`／`禮券`／`家長會` 一律禁用**；
  中秋烤肉題材 9/18 已寫完，`中秋`／`烤肉`／`月餅`／`麻糬`／`鹽可頌`／`白吐司` **全批禁用**。
